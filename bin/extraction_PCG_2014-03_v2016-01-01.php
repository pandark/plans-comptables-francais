...
shell_exec( "pdftohtml -f 176 -l 197 $nomFichier PCG.html" );

# Lecture du texte résultat
$text = file_get_contents( 'PCGs.html' );

# Extraction du texte pour la classe 8
shell_exec( "pdftohtml -f 238 -l 239 $nomFichier PCG.html" );
...
$textarray = explode( "\n", $text );
$text = implode( "\n", array_merge( array_slice( $textarray, 4, -106 ), array( '<b>Classe 8 : Comptes spéciaux</b>' ), array_slice( $textarray, -61, -60 ), array_slice( $textarray, -48, -41 ), array_slice( $textarray, -40, -32 ), array_slice( $textarray, -17, -13 ), array_slice( $textarray, -12, -11 ), array_slice( $textarray, -7, -6 ), array( '<i>890. Bilan d’ouverture', '891. Bilan de clôture</i>' ) ) );

# Retrait des changements de page
$text = preg_replace( "/ *[0-9]+ *\n *Version du 1ER Janvier 2016 *\n<hr\/?><a name=[0-9]+><\/a>PLAN COMPTABLE GENERAL VERSION CONSOLIDEE *\n/i", "\n", $text );
...
file_put_contents( 'PCG_2014-03_v2016-01-01.csv', $text . "\n" );
...
#foreach( array( 'base', 'étendu' ) as $system ) {
	...
	file_put_contents( "PCG_2014-03_v2016-01-01_$system-gnucash.xml", $output );
}
