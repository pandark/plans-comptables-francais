...
shell_exec( "pdftohtml -f 124 -l 143 $nomFichier PCG.html" );

# Lecture du texte résultat
$text = file_get_contents( 'PCGs.html' );

# Extraction du texte pour la classe 8
shell_exec( "pdftohtml -f 176 -l 177 $nomFichier PCG.html" );
...
unlink( 'PCGs.html' );
unlink( 'PCG_ind.html' );
unlink( "PCG-176_1.jpg" );
unlink( "PCG-177_1.jpg" );
for( $i=124; $i<=143; $i++ ) {
	unlink( "PCG-{$i}_1.jpg" );
}
...
$textarray = explode( "\n", $text );
$text = implode( "\n", array_merge( array_slice( $textarray, 12, -126 ), array( '<b>Classe 8 : Comptes spéciaux</b>' ), array_slice( $textarray, -64, -63 ), array_slice( $textarray, -55, -48 ), array_slice( $textarray, -47, -40 ), array_slice( $textarray, -19, -16 ), array_slice( $textarray, -14, -13 ), array_slice( $textarray, -9, -8 ), array( '<i>890. Bilan d’ouverture', '891. Bilan de clôture</i>' ) ) );

# Retrait des changements de page
$text = preg_replace( "/ *\n *Autorité des normes comptables *\n- page n°[0-9]+\/[0-9]+ *\n<hr\/?><a name=[0-9]+><\/a><img src=\"PCG-[0-9]+_[0-9]+\.jpg\" *\/>\n/i", "\n", $text );
...
$text = preg_replace( '/ENGAGEMENTS/', 'Engagements', $text );
$text = preg_replace( '/BILAN/', 'Bilan', $text );
$text = preg_replace( '/RESULTAT EN INSTANCE D’AFFECTATION/', 'Résultat en instance d’affectation', $text );
$text = preg_replace( '/ - /', ' – ', $text );

# Écriture du texte résultat
file_put_contents( 'PCG_2014-03_v2014-06-05.csv', $text . "\n" );
...
#foreach( array( 'base', 'étendu' ) as $system ) {
	...
	file_put_contents( "PCG_2014-03_v2014-06-05_$system-gnucash.xml", $output );
}
